  ...
  <div class="space-y-5">
            ...
            <div class="relative w-12 h-12 mb-2">
              ...
              >
                ...
                <circle
                  ...
                  :stroke-dashoffset="264 - (264 * s.percent / 100)"
    ...
    <div class="card p-4 rounded-xl border-neutral-border/50">
      ...
      <div class="grid grid-cols-4 sm:grid-cols-6 lg:grid-cols-8 xl:grid-cols-10 gap-2">
        <div
          v-for="(count, prof) in data.summary?.profession_distribution || {}"
          :key="prof"
        ...
        >
          ...
          <div class="flex items-center gap-3 text-xs flex-shrink-0">
            <div class="text-center">
              <span class="block font-semibold text-neutral-text">{{ fmtCompact(g.avg_dps) }}</span><span class="text-neutral-text-secondary">平均DPS</span>
            </div>
            <div class="text-center">
              <span class="block font-semibold text-error">{{ g.total_dead }}</span><span class="text-neutral-text-secondary">死亡</span>
            </div>
            <div class="text-center">
              <span class="block font-semibold text-warning">{{ g.total_downed }}</span><span class="text-neutral-text-secondary">击倒</span>
            </div>
          </div>
        </div>
      </div>
...
import type { EiAnalysisAggregate, EiAnalysisFight, EiAnalysisGroup, EiAnalysisPlayer, EiAnalysisResponse } from '@/services/ei/eiAnalysisService'
...
function teamBarStyle(g: EiAnalysisGroup) {
  const totalDmg = props.data.groups.reduce((s, x) => s + x.total_damage, 0)
  const avgDmg = totalDmg / Math.max(props.data.groups.length, 1)
  const width = Math.min((g.total_damage / Math.max(avgDmg, 1)) * 50, 100)
