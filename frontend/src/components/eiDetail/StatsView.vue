      ...
      <div class="chart-header">
        ...
        <div class="chart-tabs">
          <button
            ...
            @click="$emit('update:activeChartTab', tab.key)"
          >
            {{ tab.label }}
          </button>
        </div>
        ...
        <div class="chart-placeholder">
          <div class="chart-content">
            <div class="damage-bars">
              <div
                v-for="(player, index) in playersSortedByDmg.slice(0, 5)"
              ...
              >
                ...
                <div class="bar-container">
                  <div
                    class="bar-fill"
                    :style="{
                      width: getChartValue(player, activeChartTab) + '%',
                      backgroundColor: getProfessionColor(player.profession)
                    }"
                  />
                </div>
                <div class="bar-value">
                  {{ formatLargeNumber(getChartRawValue(player, activeChartTab)) }}
                </div>
              </div>
            </div>
          </div>
          <div class="chart-summary">
            <div class="summary-item">
              ...
              <span class="value">{{ formatLargeNumber(totalDamage) }}</span>
            </div>
            <div class="summary-item">
              <span class="label">平均DPS</span>
              <span class="value">{{ formatLargeNumber(avgDps) }}</span>
            </div>
            <div class="summary-item">
              <span class="label">战斗时长</span>
              <span class="value">{{ fightDuration }}</span>
            </div>
          </div>
        </div>
      </div>
...
import { getProfessionName as _getProfName, getProfessionColor as _getProfColor, getProfessionIconUrl } from '@/utils/profession/professionUtils'
import DamageDistributionChart from './DamageDistributionChart.vue'
import SkillCastChart from './SkillCastChart.vue'
...
const sortedPlayers = computed(() => {
  const list = [...props.playersSortedByDmg]
  switch (props.sortBy) {
    case 'damage':
...
const totalDamage = computed(() => {
  return props.playersSortedByDmg.reduce((sum, p) => sum + getPlayerDamage(p), 0)
})

const totalPowerDamage = computed(() => {
  return props.playersSortedByDmg.reduce((sum, p) => sum + getPlayerPowerDamage(p), 0)
})

const totalCondiDamage = computed(() => {
  return props.playersSortedByDmg.reduce((sum, p) => sum + getPlayerCondiDamage(p), 0)
})

const totalBreakbarDamage = computed(() => {
  return props.playersSortedByDmg.reduce((sum, p) => sum + (p.dpsAll?.[0]?.breakbarDamage || 0), 0)
})
...
const alivePlayers = computed(() => {
  return props.playersSortedByDmg.filter(p => p.deaths === 0).length
})
...
function getChartValue(player: Player, tab: string): number {
  const maxValue = Math.max(...props.playersSortedByDmg.map(p => getChartRawValue(p, tab)), 1)
  return (getChartRawValue(player, tab) / maxValue) * 100
}

function getChartRawValue(player: Player, tab: string): number {
  switch (tab) {
    case 'damage':
    ...
    case 'dps':
      return player.dps
    case 'power':
      return getPlayerPowerDamage(player)
    case 'condi':
      return getPlayerCondiDamage(player)
    default:
      return getPlayerDamage(player)
  }
}

function getProfessionName(prof: string): string {
  return _getProfName(prof)
}

function getProfessionColor(prof: string): string {
  return _getProfColor(prof)
}
