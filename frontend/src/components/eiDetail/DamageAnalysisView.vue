<template>
  <div class="damage-analysis-view space-y-6">
    ...
    <div class="card">
      ...
      >
        <Column field="rank" header="排名" style="width: 60px">
          <template #body="{ data }">
            <div class="flex items-center justify-center w-7 h-7 rounded-full font-bold text-xs"
              :class="{
        ...
        <Column field="name" header="玩家">
          <template #body="{ data }">
            <div class="flex items-center gap-2">
              ...
              <div>
                <div class="text-sm font-medium text-neutral-text">{{ data.name }}</div>
                <div class="text-xs text-neutral-text-secondary">{{ data.profession }}</div>
              </div>
            </div>
          </template>
        </Column>
        <Column field="damage" header="总伤害" sortable>
          <template #body="{ data }">
            <span class="text-primary font-semibold">{{ formatNumber(data.damage) }}</span>
          </template>
        </Column>
        <Column field="powerDamage" header="直伤" sortable>
          <template #body="{ data }">
            <span class="text-neutral-text">{{ formatNumber(data.powerDamage) }}</span>
          </template>
        </Column>
        <Column field="condiDamage" header="症状伤" sortable>
          <template #body="{ data }">
            <span class="text-status-warning">{{ formatNumber(data.condiDamage) }}</span>
          </template>
        </Column>
        <Column field="dps" header="DPS" sortable>
          <template #body="{ data }">
            <span class="font-semibold">{{ formatNumber(data.dps) }}</span>
          </template>
        </Column>
        <Column field="damageTaken" header="承伤" sortable>
          <template #body="{ data }">
    ...
    <!-- 目标伤害 -->
    <div v-if="targets.length > 0" class="card">
      <h3 class="text-lg font-semibold text-neutral-text mb-4">
      ...
      <div class="space-y-3">
        ...
        >
          ...
          <div class="text-right">
            <p class="font-semibold text-primary">{{ formatNumber(target.dpsAll?.[0]?.damageTaken || 0) }}</p>
