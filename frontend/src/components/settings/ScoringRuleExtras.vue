<template>
  <div class="mb-8 p-4 rounded-xl border-2 border-dashed border-neutral-border hover:border-primary/50 transition-colors">
    <div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
      <div class="md:col-span-3">
        ...
        <BaseSelect v-model="localDimension" :options="availableDimensions" option-label="label" option-value="key" placeholder="ѡ择维度" size="small" class="w-full">
          <template #option="{ option }">
            <div class="flex items-center gap-2">
              <div class="w-5 h-5 rounded bg-neutral-bg-secondary flex items-center justify-center text-white text-xs" :style="{ background: getDimensionColor(option.key) }">
      ...
      <div class="md:col-span-2">
        ...
        <InputNumber v-model="localWeight" :min="0" :max="100" :step="1" size="small" class="w-full" />
      </div>
      <div class="md:col-span-5">
        <label class="block text-xs font-medium text-neutral-text-secondary mb-1.5">描述</label>
        <InputText v-model="localDesc" size="small" class="w-full" placeholder="输入规则描述..." />
      </div>
      <div class="md:col-span-2">
        <BaseButton label="添加" icon="pi pi-plus" severity="success" size="small" :disabled="!localDimension" class="w-full" @click="handleAdd" />
      </div>
    </div>
  </div>

  <div class="mb-8 p-4 rounded-xl bg-neutral-bg-secondary">
    <h4 class="text-sm font-semibold text-neutral-text mb-4 flex items-center gap-2"><i class="pi pi-info-circle text-primary" />评分等级说明</h4>
    <div class="grid grid-cols-6 gap-3">
      <div v-for="grade in GRADE_LEVELS" :key="grade.letter" class="text-center p-3 rounded-lg bg-neutral-card">
        <div class="text-2xl font-bold mb-1" :style="{ background: grade.gradient, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }">{{ grade.letter }}</div>
  ...
  <div class="flex justify-end gap-3 pt-6 border-t border-neutral-border">
    <BaseButton label="重置默认" icon="pi pi-refresh" severity="secondary" @click="$emit('reset-default')" />
    <BaseButton label="ȡ消" icon="pi pi-times" severity="secondary" outlined @click="$emit('cancel')" />
    <BaseButton label="保存更改" icon="pi pi-check" severity="primary" :disabled="!hasChanges" @click="$emit('save')" />
  </div>
</template>
...
import type { DimensionInfo } from '@/services/scoring/scoringRulesService'
