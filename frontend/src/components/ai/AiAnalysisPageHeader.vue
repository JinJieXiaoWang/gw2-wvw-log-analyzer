    ...
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
      <div class="flex items-center gap-4">
        ...
        <div>
          ...
          <p class="text-neutral-text-secondary mt-1.5">
            {{ pageSubtitle }}
          </p>
        </div>
      </div>
      <div class="flex items-center gap-3">
        <button
          ...
          class="flex items-center gap-2 px-5 py-2.5 bg-neutral-card hover:bg-neutral-card-hover border border-neutral-border rounded-xl transition-all"
          @click="$emit('refresh')"
        >
          <SvgIcon
            v-if="!isRefreshing"
            icon="refresh-cw"
            :size="18"
            class="text-neutral-text-secondary"
          />
          <span
            v-else
            class="animate-spin"
        ...
        <button
          class="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-primary to-indigo-600 hover:from-primary-light hover:to-indigo-500 text-white rounded-xl transition-all font-medium"
