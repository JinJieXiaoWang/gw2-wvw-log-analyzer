<template>
  <div class="space-y-4" :class="{ 'opacity-50': disabled }">
    <!-- 加载状态 -->
    <div v-if="loading" class="flex flex-col items-center justify-center py-12">
      ...
      <p class="text-gray-400 text-sm">AI正在生成优化建议...</p>
    </div>

    <!-- 空状态 -->
    <div v-else-if="!suggestionsData || suggestionsData.suggestions.length === 0" class="text-center py-12">
      <div class="inline-flex items-center justify-center p-4 bg-gray-700/50 rounded-xl mb-4">
        <SvgIcon icon="lightbulb" :size="32" class="text-gray-500" />
      </div>
      <p class="text-gray-400 mb-2">暂无优化建议</p>
      <p class="text-sm text-gray-500">配置AI后可获取智能优化建议</p>
    </div>

    <!-- 建议列表 -->
    <div v-else class="space-y-4">
      <!-- 高优先级建议 -->
      <div v-if="suggestionsData.high_priority && suggestionsData.high_priority.length > 0" class="space-y-3">
        <div class="flex items-center gap-2">
          <div class="p-1.5 bg-red-500/20 rounded-lg">
            <SvgIcon icon="alert-triangle" :size="14" class="text-red-400" />
          </div>
          <h3 class="text-sm font-semibold text-red-400">高优先级建议</h3>
        ...
        >
          <div class="flex items-start gap-3">
            <div class="p-2 bg-red-500/20 rounded-lg flex-shrink-0">
              <SvgIcon icon="flame" :size="16" class="text-red-400" />
            </div>
            <div class="flex-1">
      ...
      <div class="space-y-3">
        <div class="flex items-center gap-2">
          <div class="p-1.5 bg-blue-500/20 rounded-lg">
            <SvgIcon icon="lightbulb" :size="14" class="text-blue-400" />
          </div>
          <h3 class="text-sm font-semibold text-blue-400">优化建议</h3>
        ...
        >
          <div class="flex items-start gap-3">
            <div class="p-2 bg-blue-500/20 rounded-lg flex-shrink-0 group-hover:bg-blue-500/30 transition-colors">
              <SvgIcon icon="sparkles" :size="16" class="text-blue-400" />
            </div>
            <div class="flex-1">
              <p class="text-gray-200 leading-relaxed">{{ suggestion }}</p>
              <div class="mt-3 flex items-center gap-2">
                <button 
      ...
      <div class="pt-4 border-t border-gray-700/50">
        <div class="flex items-center justify-between text-xs text-gray-500">
          ...
          >
            <SvgIcon icon="refresh-cw" :size="12" />
            <span>获取更多建议</span>
          </button>
        </div>
      </div>
