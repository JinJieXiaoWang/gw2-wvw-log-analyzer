  ...
  <div class="growth-tab">
    <div class="mb-6 flex flex-col sm:flex-row gap-4 items-start sm:items-end">
      ...
      <div>
        ...
        <select :value="growthFightCount" @change="$emit('update:growthFightCount', Number(($event.target as HTMLSelectElement).value))" class="bg-neutral-card-active text-white px-4 py-2.5 rounded-lg border border-neutral-border">
          <option v-for="opt in HISTORY_OPTIONS" :key="opt.value" :value="opt.value">{{ opt.label }}</option>
        </select>
      </div>
      <button @click="$emit('run')" :disabled="!selectedPlayerAccount || loadingPersonalGrowth" class="px-6 py-2.5 bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-400 hover:to-teal-400 text-white rounded-lg font-medium transition-all disabled:opacity-50">
