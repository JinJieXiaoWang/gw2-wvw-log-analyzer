<template>
  <div>
    <div class="flex items-center gap-3 mb-6">
      <div class="p-2 bg-gradient-to-br from-primary/20 to-indigo-500/20 rounded-xl">
        <SvgIcon
          icon="wrench"
          :size="24"
          class="text-primary"
    ...
    <div class="space-y-4">
      <div>
        ...
        <select
          ...
          class="w-full bg-neutral-card-active text-white text-sm px-4 py-2 rounded-lg border border-neutral-border"
        >
          <option value="">
            请选择战斗
          </option>
      ...
      <div>
        ...
        >
          ...
          <option
            ...
            :value="player.id"
          >
            {{ player.name }} - {{ getProfessionName(player.profession) }}
          </option>
        </select>
      </div>
      <div>
        ...
        >
          ...
          <option
            ...
            :value="build.id"
          >
            {{ build.name }} - {{ getProfessionName(build.profession) }}
          </option>
        </select>
      </div>
      <div class="flex items-center gap-2 pt-2">
        <input
          id="scope-current"
          v-model="useCurrentScope"
          type="checkbox"
          class="w-4 h-4 rounded border-neutral-border text-primary focus:ring-primary"
        >
        <label
          for="scope-current"
      ...
      <button
        ...
        class="w-full px-5 py-3 bg-gradient-to-r from-primary to-indigo-600 hover:from-primary-light hover:to-indigo-500 disabled:from-neutral-card disabled:to-neutral-card disabled:cursor-not-allowed text-white rounded-xl transition-all font-medium"
        @click="handleAnalyze"
      >
        {{ analyzingText }}
      </button>
