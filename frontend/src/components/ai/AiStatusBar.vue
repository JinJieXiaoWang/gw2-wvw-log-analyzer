<template>
  <div class="mt-6 flex flex-wrap items-center gap-3">
    <div class="flex items-center gap-2 px-4 py-2.5 bg-neutral-card/80 backdrop-blur-sm rounded-xl border border-neutral-border hover:border-ai/50 transition-all">
      <div class="w-2.5 h-2.5 rounded-full relative" :class="config.enabled ? 'bg-ai' : 'bg-error'">
        <div v-if="config.enabled" class="absolute inset-0 bg-ai rounded-full animate-ping opacity-50" />
      </div>
      <span class="text-sm font-medium" :class="config.enabled ? 'text-ai' : 'text-error'">
    ...
    <div class="flex items-center gap-2 px-4 py-2.5 bg-neutral-card/80 backdrop-blur-sm rounded-xl border border-neutral-border">
      <SvgIcon icon="zap" :size="16" class="text-ai" />
