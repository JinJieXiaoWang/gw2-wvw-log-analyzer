    ...
    <div v-if="buildExecutionData">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
        ...
        <div class="p-4 bg-neutral-card-active/40 rounded-xl border border-neutral-border">
          ...
          <div class="text-lg font-semibold" :class="getScoreClass(ruleResult.overallScore)">{{ ruleResult.overallScore }}</div>
        </div>
        <div class="p-4 bg-neutral-card-active/40 rounded-xl border border-neutral-border">
          <div class="text-xs text-neutral-text-secondary mb-1">规则通过</div>
          <div class="text-lg font-semibold text-status-success">{{ totalPassed }} / {{ ruleResult.rules.length }}</div>
        </div>
      </div>

      <!-- 分类汇总标签 -->
      <div v-if="ruleResult.rules.length" class="flex flex-wrap gap-2 mb-4">
        <div v-for="(summary, cat) in ruleResult.categorySummary" :key="cat" class="flex items-center gap-1.5 px-3 py-1.5 bg-neutral-card-active/30 rounded-lg text-xs">
          <span class="text-neutral-text-secondary">{{ CATEGORY_LABELS[cat as RuleCategory] }}</span>
          <span v-if="summary.pass" class="text-status-success">{{ summary.pass }}✓</span>
          <span v-if="summary.fail" class="text-error">{{ summary.fail }}✗</span>
          <span v-if="summary.warn" class="text-warning">{{ summary.warn }}!</span>
        </div>
      </div>

      <!-- 规则检查列表 -->
      <div v-if="ruleResult.rules.length" class="space-y-2">
        <div v-for="rule in ruleResult.rules" :key="rule.id" class="flex items-start justify-between p-3 bg-neutral-card-active/30 rounded-lg">
          <div class="flex-1 min-w-0">
            ...
            <p class="text-xs text-neutral-text-secondary">{{ rule.description }}</p>
            <p v-if="rule.actual" class="text-xs text-neutral-text-tertiary mt-0.5">{{ LABEL_ACTUAL_PREFIX }} {{ rule.actual }}</p>
          </div>
          <span class="text-xs px-2 py-0.5 rounded font-medium shrink-0 ml-2" :class="getRuleStatusClass(rule.status)">{{ rule.status.toUpperCase() }}</span>
        </div>
      </div>
    </div>
