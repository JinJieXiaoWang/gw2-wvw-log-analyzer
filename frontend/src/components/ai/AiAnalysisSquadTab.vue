<template>
  <div class="squad-tab">
    <div class="mb-6 flex flex-col sm:flex-row gap-4 items-start sm:items-end">
      ...
      <button
        :disabled="!selectedFightId || loadingSquadSynergy"
        class="px-6 py-2.5 bg-gradient-to-r from-blue-500 to-indigo-500 hover:from-blue-400 hover:to-indigo-400 text-white rounded-lg font-medium transition-all disabled:opacity-50"
        @click="$emit('run')"
      >
    ...
    <div
      v-if="squadSynergyData"
      class="space-y-4"
    >
      <div
        v-for="sq in squadSynergyData.squads || []"
        :key="sq.group_id"
        class="p-4 bg-neutral-card-active/40 rounded-xl border border-neutral-border"
      >
        <div class="flex items-center justify-between mb-3">
          <h3 class="font-semibold text-white">
            {{ LABEL_SQUAD_PREFIX }} {{ sq.group_id }}
          </h3>
          <span
            class="text-lg font-bold"
            :class="getScoreClass(sq.synergy_score)"
          >{{ sq.synergy_score }}</span>
        </div>
        <div class="text-sm text-neutral-text-secondary mb-2">
          {{ sq.member_count }}{{ LABEL_SQUAD_MEMBERS_SUFFIX }} {{ formatRoleDistribution(sq.role_distribution) }}
        </div>
        <div
          v-if="sq.suggestions?.length"
          class="space-y-1.5 mt-3"
        >
          <div
            v-for="(s, i) in sq.suggestions"
            :key="i"
            class="text-xs p-2 rounded"
            :class="s.priority === 'high' ? 'bg-error/10 text-error' : 'bg-warning/10 text-warning'"
          >
            {{ s.message }}
          </div>
        </div>
