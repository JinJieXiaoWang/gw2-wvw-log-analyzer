    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      >
        <div class="flex items-start gap-3">
          <div class="w-12 h-12 rounded-xl flex items-center justify-center text-white font-bold text-lg shrink-0" :style="{ background: getProfessionGradient(char.profession) }">
            {{ char.character_name.charAt(0) }}
          </div>
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2 mb-1">
              <span class="font-semibold text-neutral-text truncate">{{ char.character_name }}</span>
              <span class="text-xs px-2 py-0.5 rounded-full" :style="{ backgroundColor: getProfessionColorVal(char.profession) + '30', color: getProfessionColorVal(char.profession) }">
                {{ getProfessionLabel(char.profession) }}
              </span>
            </div>
            <div class="grid grid-cols-2 gap-2 text-xs">
              <div><span class="text-neutral-text-secondary">出勤</span><p class="font-bold text-primary">{{ char.attendance_count }}天</p></div>
              <div><span class="text-neutral-text-secondary">DPS</span><p class="font-bold text-status-error">{{ formatDps(char.avg_dps) }}</p></div>
              <div><span class="text-neutral-text-secondary">伤害</span><p class="font-bold text-status-error">{{ formatNumber(char.total_damage) }}</p></div>
              <div><span class="text-neutral-text-secondary">评分</span><span :class="getScoreColor(char.avg_score)" class="font-bold">{{ char.avg_score }}</span></div>
            </div>
          </div>
        </div>
        <div class="mt-3 pt-3 border-t border-neutral-border/30">
          <div class="flex items-center justify-between text-xs">
            ...
            <span :class="char.kd_ratio >= 2 ? 'text-status-success' : char.kd_ratio >= 1 ? 'text-primary' : 'text-status-error'" class="font-semibold">{{ char.kd_ratio }}</span>
          </div>
        </div>
      </div>
    </div>
...
import { getProfessionLabel, getScoreColor, formatNumber, formatDps } from '@/utils/attendance/attendanceFormatters'
