    ...
    <div v-if="isOpen" class="color-panel" @click.stop>
      ...
      <div class="custom-section">
        ...
        <div class="custom-input-row">
          ...
          <InputText v-model="localCustomHex" placeholder="RRGGBB" class="flex-1" maxlength="6" @blur="$emit('apply-custom')" @keydown.enter="$emit('apply-custom')" />
          <div class="custom-preview" :style="{ backgroundColor: isValidCustomHex ? '#' + localCustomHex : undefined }" />
        </div>
...
const emit = defineEmits<{
  'update:customHexInput': [value: string]
  select: [color: string]
  clear: []
  close: []
  'apply-custom': []
...
const localCustomHex = computed({
  get: () => props.customHexInput,
  set: v => emit('update:customHexInput', v)
})
</script>
