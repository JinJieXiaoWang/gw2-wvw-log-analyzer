  ...
  <Card class="border-none bg-surface-100/50">
    <template #content>
      <div class="flex items-start gap-4">
        <div
          class="w-14 h-14 rounded-full flex-shrink-0 overflow-hidden"
          :style="{ backgroundColor: color + '30' }"
        >
          <img
            :src="getProfessionIconUrl(player.profession)"
            :alt="getProfessionName(player.profession)"
        ...
        <div class="flex-1 min-w-0">
          <div class="flex items-center gap-2 mb-1">
            ...
            <Badge
              v-if="player.score_grade"
              :severity="getScoreSeverity(player.score_grade)"
              class="text-xs"
            >
              {{ player.score_grade }}
            </Badge>
          ...
          <p class="text-sm text-neutral-text-secondary flex items-center gap-1">
            <i class="pi pi-users text-xs" />{{ getProfessionName(player.profession) }}
          </p>
        </div>
        <div class="grid grid-cols-3 gap-4">
          <div class="text-center p-2 rounded-lg bg-surface-200/50">
            <p class="text-xl font-bold text-primary">
              {{ fmtCompact(player.damage) }}
            </p><p class="text-xs text-neutral-text-secondary mt-1">
              总伤害
            </p>
          </div>
          <div class="text-center p-2 rounded-lg bg-surface-200/50">
