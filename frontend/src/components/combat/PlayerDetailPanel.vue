<template>
  <Dialog :visible="visible" :header="headerTitle" :style="{ width: '900px', maxWidth: '95vw' }" :modal="true" :draggable="false" :resizable="true" class="player-detail-dialog" @hide="emit('update:visible', false)">
    <div v-if="player" class="space-y-5">
      <PlayerDetailHeader :player="player" />
      <LoadingState v-if="loading" text="加载技能数据中..." />
      <PlayerDetailRotation v-else-if="rotation" :rotation="rotation" :events="events" :fight-duration="fightDuration" :loading="loading" @refresh="emit('refresh')" />
      <BaseState v-else icon="pi pi-file-text" title="该日志未生成技能循环数据" description="请重新解析日志以获取技能详情" />
    </div>
  </Dialog>
</template>
...
import BaseState from '@/components/common/ui/BaseState.vue'
