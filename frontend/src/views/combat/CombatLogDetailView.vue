<template>
  <div class="space-y-6">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div class="flex-1">
          <div class="flex items-center gap-3 mb-3">
            <router-link to="/logs" class="p-2 rounded-lg bg-neutral-card/50 hover:bg-primary/10 transition-all duration-200 text-neutral-text-secondary hover:text-primary group">
              <i class="pi pi-arrow-left group-hover:translate-x-[-2px] transition-transform" />
            </router-link>
            <div>
              <h1 class="text-2xl sm:text-3xl font-bold text-neutral-text tracking-tight">战斗分析</h1>
        ...
        <div class="flex flex-wrap gap-3">
          <Button label="重新解析" icon="pi pi-refresh" :loading="parsing" @click="reparseLog"
    ...
    <!-- 加载/错误 -->
    <div v-if="loading" class="card flex items-center justify-center py-16 bg-neutral-card/50 border-neutral-border/50">
    ...
    <div v-else-if="error" class="card bg-error/10 border-error/30 text-error p-6 rounded-xl">
      <div class="flex items-center gap-4">
        <div class="p-3 rounded-lg bg-error/20">
          <i class="pi pi-exclamation-triangle text-2xl" />
        </div>
        <div>
          <p class="font-semibold text-lg">加载失败</p>
    ...
    <template v-else>
        ...
        <div class="flex flex-wrap items-center justify-between gap-4">
          <div class="flex flex-wrap items-center gap-6">
            <div class="flex items-center gap-2 group cursor-pointer hover:text-primary transition-colors">
              ...
              <div>
                <p class="text-xs text-neutral-text-secondary">战斗时长</p>
                <p class="text-sm font-semibold text-neutral-text">{{ fmtDuration(fightSummary.duration_sec || 0) }}</p>
              </div>
            </div>
            <div class="flex items-center gap-2 group cursor-pointer hover:text-primary transition-colors">
              ...
              <div>
                <p class="text-xs text-neutral-text-secondary">参战人数</p>
                <p class="text-sm font-semibold text-neutral-text">{{ summary?.total_players || 0 }} 人</p>
              </div>
            </div>
            <div class="flex items-center gap-2 group cursor-pointer hover:text-primary transition-colors">
              ...
              <div>
                <p class="text-xs text-neutral-text-secondary">地图</p>
                <p class="text-sm font-semibold text-neutral-text">{{ fightSummary.map_name || '-' }}</p>
              </div>
            </div>
            <div class="flex items-center gap-2 group cursor-pointer hover:text-primary transition-colors">
              ...
              <div>
                <p class="text-xs text-neutral-text-secondary">上传时间</p>
                <p class="text-sm font-semibold text-neutral-text">{{ fmtDate(logDetail.upload_time) }}</p>
              </div>
            </div>
          </div>
          <div class="flex items-center gap-3">
            <a v-if="summary?.dps_report_permalink" :href="summary.dps_report_permalink" target="_blank" class="no-underline">
              <Tag value="EI报告" icon="pi pi-external-link" severity="info" class="text-xs cursor-pointer hover:bg-info/30 transition-all" />
            </a>
            <Tag :value="`击杀 ${fightSummary.kill_count || 0}`" severity="success" class="text-xs px-2 py-1" />
            <Tag :value="`死亡 ${fightSummary.death_count || 0}`" severity="danger" class="text-xs px-2 py-1" />
            <Tag v-if="agg.player_count" :value="`平均DPS ${fmtCompact(agg.avg_dps)}`" severity="info" class="text-xs px-2 py-1" />
          </div>
        </div>
      </div>
      ...
      <div class="card p-0 overflow-hidden">
        <TabMenu v-model:active-index="activeTab" :model="tabItems" />
        <div class="p-5">
          ...
          <div v-if="activeTab === 0" class="space-y-5">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
              <div v-for="(k, idx) in kpiList" :key="k.label" 
                ...
                <div class="flex items-center justify-between mb-3">
                  ...
                  <div class="p-2 rounded-lg bg-white/5 group-hover:bg-white/10 transition-colors">
                    <i :class="k.icon + ' ' + k.color" class="text-lg" />
                  </div>
                </div>
                ...
                <div class="mt-3 h-1 bg-white/10 rounded-full overflow-hidden">
                  <div class="h-full rounded-full transition-all duration-700" :class="k.barColor" :style="{ width: k.percent + '%' }" />
                </div>
              </div>
            </div>
            ...
            <div class="card p-5 rounded-xl border-neutral-border/50">
              ...
              <div class="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-7 gap-3">
                <!-- 伤害构成 -->
                <div class="card p-3 rounded-xl border-primary/20 bg-gradient-to-br from-primary/5 to-transparent hover:border-primary/40 hover:shadow-lg hover:shadow-primary/10 transition-all duration-300 cursor-pointer"
                  ...
                  <div class="flex flex-col items-center">
                    <div class="relative w-12 h-12 mb-2">
                      <svg viewBox="0 0 100 100" class="w-full h-full -rotate-90">
                        ...
                        <circle cx="50" cy="50" r="42" fill="none" stroke="#165DFF" stroke-width="8"
                          :stroke-dasharray="donut.pd" />
                        <circle cx="50" cy="50" r="42" fill="none" stroke="#22c55e" stroke-width="8"
                          :stroke-dasharray="donut.cd" :stroke-dashoffset="donut.co" />
                      </svg>
                      <div class="absolute inset-0 flex items-center justify-center">
                        <i class="pi pi-chart-pie text-primary text-sm" />
                      </div>
                    </div>
                    <p class="text-lg font-bold text-neutral-text">{{ fmtCompact(donut.total) }}</p>
                    <p class="text-xs text-neutral-text-secondary">伤害构成</p>
                  </div>
                </div>

                <!-- 保护覆盖率 -->
                <div class="card p-3 rounded-xl border-info/20 bg-gradient-to-br from-info/5 to-transparent hover:border-info/40 hover:shadow-lg hover:shadow-info/10 transition-all duration-300 cursor-pointer"
                  ...
                  <div class="flex flex-col items-center">
                    ...
                    <p class="text-lg font-bold text-neutral-text">{{ statAverages.protection.toFixed(0) }}%</p>
                    <p class="text-xs text-neutral-text-secondary">保护</p>
                  </div>
                </div>

                <!-- 稳固覆盖率 -->
                <div class="card p-3 rounded-xl border-warning/20 bg-gradient-to-br from-warning/5 to-transparent hover:border-warning/40 hover:shadow-lg hover:shadow-warning/10 transition-all duration-300 cursor-pointer"
                  ...
                  <div class="flex flex-col items-center">
                    ...
                    <p class="text-lg font-bold text-neutral-text">{{ statAverages.stability.toFixed(0) }}%</p>
                    <p class="text-xs text-neutral-text-secondary">稳固</p>
                  </div>
                </div>

                <!-- 清症总数 -->
                <div class="card p-3 rounded-xl border-success/20 bg-gradient-to-br from-success/5 to-transparent hover:border-success/40 hover:shadow-lg hover:shadow-success/10 transition-all duration-300 cursor-pointer"
                  ...
                  <div class="flex flex-col items-center">
                    ...
                    <p class="text-lg font-bold text-neutral-text">{{ fmtCompact(agg.total_condition_cleanses) }}</p>
                    <p class="text-xs text-neutral-text-secondary">清症</p>
                  </div>
                </div>

                <!-- 削增益总数 -->
                <div class="card p-3 rounded-xl border-error/20 bg-gradient-to-br from-error/5 to-transparent hover:border-error/40 hover:shadow-lg hover:shadow-error/10 transition-all duration-300 cursor-pointer"
                  ...
                  <div class="flex flex-col items-center">
                    ...
                    <p class="text-lg font-bold text-neutral-text">{{ fmtCompact(agg.total_boon_strips) }}</p>
                    <p class="text-xs text-neutral-text-secondary">削增益</p>
                  </div>
                </div>

                <!-- 承伤总量 -->
                <div class="card p-3 rounded-xl border-secondary/20 bg-gradient-to-br from-secondary/5 to-transparent hover:border-secondary/40 hover:shadow-lg hover:shadow-secondary/10 transition-all duration-300 cursor-pointer"
                  ...
                  <div class="flex flex-col items-center">
                    ...
                    <p class="text-lg font-bold text-neutral-text">{{ fmtCompact(agg.total_damage_taken) }}</p>
                    <p class="text-xs text-neutral-text-secondary">承伤</p>
                  </div>
                </div>
                ...
                <div class="card p-3 rounded-xl border-primary/20 bg-gradient-to-br from-primary/5 to-transparent hover:border-primary/40 hover:shadow-lg hover:shadow-primary/10 transition-all duration-300 cursor-pointer"
                  ...
                  <div class="flex flex-col items-center">
                    ...
                    <p class="text-lg font-bold text-neutral-text">{{ statAverages.hitRate.toFixed(1) }}%</p>
                    <p class="text-xs text-neutral-text-secondary">命中率</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="card p-4 rounded-xl border-neutral-border/50">
              ...
              <div class="grid grid-cols-4 sm:grid-cols-6 lg:grid-cols-8 xl:grid-cols-10 gap-2">
                <div v-for="(count, prof) in summary?.profession_distribution" :key="prof" 
                  ...
                  <span class="text-[10px] text-center text-neutral-text-secondary" :style="{ backgroundColor: getProfessionColor(prof) + '20', color: getProfessionColor(prof) }">{{ getProfessionName(prof) }}</span>
                </div>
              </div>
            </div>
            ...
            <div class="card p-5 rounded-xl border-neutral-border/50">
              <div class="flex items-center justify-between mb-5">
                ...
                <Button :label="showDetailStats ? '收起' : '展开'" :icon="showDetailStats ? 'pi pi-chevron-up' : 'pi pi-chevron-down'" 
                  severity="secondary" size="small" class="transition-all duration-200" @click="showDetailStats = !showDetailStats" />
              </div>
              
              <div v-show="showDetailStats" class="space-y-5">
                <!-- 伤害细分 -->
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
                  <div class="card p-4 rounded-xl bg-gradient-to-br from-primary/10 to-transparent border-primary/20">
                    ...
                    <div class="text-center">
                      <p class="text-3xl font-bold text-primary">{{ fmtCompact(agg.total_power_damage) }}</p>
                      <div class="mt-2 flex items-center justify-center gap-2">
                        <div class="w-20 h-2 bg-neutral-bg rounded-full overflow-hidden">
                          <div class="h-full bg-primary rounded-full transition-all duration-700" :style="{ width: powerPct + '%' }" />
                        </div>
                  ...
                  <div class="card p-4 rounded-xl bg-gradient-to-br from-success/10 to-transparent border-success/20">
                    ...
                    <div class="text-center">
                      <p class="text-3xl font-bold text-success">{{ fmtCompact(agg.total_condi_damage) }}</p>
                      <div class="mt-2 flex items-center justify-center gap-2">
                        <div class="w-20 h-2 bg-neutral-bg rounded-full overflow-hidden">
                          <div class="h-full bg-success rounded-full transition-all duration-700" :style="{ width: condiPct + '%' }" />
                        </div>
                  ...
                  <div class="card p-4 rounded-xl bg-gradient-to-br from-secondary/10 to-transparent border-secondary/20">
                    ...
                    <div class="text-center">
                      <p class="text-3xl font-bold text-secondary">{{ fmtCompact(agg.total_breakbar_damage) }}</p>
                      <div class="mt-2 flex items-center justify-center gap-2">
                        <div class="w-20 h-2 bg-neutral-bg rounded-full overflow-hidden">
                          <div class="h-full bg-secondary rounded-full transition-all duration-700" :style="{ width: breakbarPct + '%' }" />
                        </div>
                ...
                <!-- 详细数据表格 -->
                <DataTable :value="summary?.players || []" striped-rows :paginator="true" :rows="10" class="w-full" scrollable>
                  ...
                  <Column field="breakbar_damage" header="破甲" style="min-width: 90px">
                    <template #body="{ data }">
                      <span class="text-sm">{{ fmtCompact(data.breakbar_damage) }}</span>
                    </template>
                  </Column>
                  <Column field="flanking_rate" header="侧身率" style="min-width: 80px">
                    <template #body="{ data }">
                      <span class="text-sm">{{ data.flanking_rate.toFixed(1) }}%</span>
                    </template>
                  </Column>
                  <Column field="glance_rate" header="擦过率" style="min-width: 80px">
                    <template #body="{ data }">
                      <span class="text-sm">{{ data.glance_rate.toFixed(1) }}%</span>
                    </template>
                  </Column>
                  <Column field="missed" header="未命中" style="min-width: 80px" />
                  ...
                  <Column field="condition_cleanses" header="清症" style="min-width: 70px" />
                  <Column field="resurrects" header="复活" style="min-width: 70px" />
                  <Column field="healing" header="治疗" style="min-width: 90px">
                    <template #body="{ data }">
          ...
          <div v-if="activeTab === 1" class="space-y-5">
            ...
            <div class="flex items-center gap-3">
              ...
              <h3 class="text-lg font-semibold text-neutral-text">玩家排行 & 小队编制</h3>
              <Tag :value="`${players.length}人`" severity="info" class="text-xs" />
            </div>
            ...
            <div class="grid grid-cols-1 xl:grid-cols-5 gap-5">
              ...
              <div class="xl:col-span-3 card rounded-xl border-neutral-border/50 overflow-hidden">
                ...
                <div class="max-h-[600px] overflow-auto">
                  <DataTable :value="sortedPlayerList" :paginator="true" :rows="12" class="w-full cursor-pointer" 
                    @row-click="onRowClick" stripedRows>
                    <Column field="rank" header="#" style="width: 50px">
                      <template #body="{ index }">
                        <span class="w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold" :class="rankClass(index)">{{ index + 1 }}</span>
                      </template>
                    </Column>
                    <Column field="profession" header="职业" style="min-width: 90px">
                      <template #body="{ data }">
                        <div class="flex items-center gap-2">
                          <img :src="getProfessionIconUrl(data.profession)" class="w-6 h-6 rounded-full" />
                          <span class="text-xs" :style="{ backgroundColor: getProfessionColor(data.profession) + '20', color: getProfessionColor(data.profession) }">{{ getProfessionName(data.profession) }}</span>
                        </div>
                      </template>
                    </Column>
                    <Column field="name" header="玩家" style="min-width: 120px">
                      <template #body="{ data }">
                        <div class="flex items-center gap-2">
                          <span class="text-xs font-medium text-neutral-text truncate max-w-[100px]">{{ data.character_name || data.account }}</span>
                          <Tag v-if="data.has_commander_tag" icon="pi pi-star-fill" severity="warn" value="" class="text-[8px] w-4 h-4 p-0 flex items-center justify-center" />
                        </div>
                      </template>
                    </Column>
                    <Column field="group_id" header="小队" style="width: 60px">
                      <template #body="{ data }">
                    ...
                    <Column field="dps" header="DPS" style="width: 80px">
                      <template #body="{ data }">
                        <span class="text-xs font-semibold text-primary">{{ fmtCompact(data.dps) }}</span>
                      </template>
                    </Column>
                    <Column field="damage" header="伤害" style="min-width: 90px">
                      <template #body="{ data }">
                        <span class="text-xs font-bold text-neutral-text">{{ fmtCompact(data.damage) }}</span>
                      </template>
                    </Column>
                    <Column field="killed" header="击杀" style="width: 50px">
                      <template #body="{ data }">
                        <span class="text-xs font-semibold text-success">{{ data.killed || 0 }}</span>
                      </template>
                    </Column>
                    <Column field="dead_count" header="死亡" style="width: 50px">
                      <template #body="{ data }">
                        <span :class="data.dead_count > 0 ? 'text-error font-semibold' : 'text-neutral-text-secondary'" class="text-xs">{{ data.dead_count || 0 }}</span>
                      </template>
                    </Column>
                    <Column field="score_grade" header="评分" style="width: 60px">
                      <template #body="{ data }">
                        <Tag :value="data.score_grade || '-'" :severity="scoreSeverity(data.score_grade)" class="text-[10px] px-1" />
                      </template>
                    </Column>
              ...
              <div class="xl:col-span-2 space-y-4">
                ...
                <div class="card p-4 rounded-xl border-yellow-500/20 bg-gradient-to-br from-yellow-500/5 to-transparent">
                  ...
                  <div class="flex flex-wrap gap-2">
                    <div v-for="cmd in commanders" :key="cmd.id"
                      ...
                      <img :src="getProfessionIconUrl(cmd.profession)" class="w-8 h-8 rounded-full border border-yellow-500/50" />
                      <div class="min-w-0">
                        <p class="text-xs font-semibold text-neutral-text truncate max-w-[80px]">{{ cmd.character_name || cmd.account }}</p>
                        <p class="text-[10px] text-yellow-500/80">{{ getProfessionName(cmd.profession) }}</p>
                      </div>
                    </div>
                    <div v-if="commanders.length === 0" class="flex items-center gap-2 text-neutral-text-secondary text-xs px-3 py-2 rounded-lg bg-neutral-bg-secondary">
                ...
                <div class="space-y-3">
                  <div v-for="g in groups" :key="g.id" 
                    ...
                    <div class="flex items-center justify-between mb-2">
                      <div class="flex items-center gap-2">
                        <span class="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold text-white" 
                          :style="{ backgroundColor: groupColor(g.id) }">{{ g.id || '?' }}</span>
                      ...
                      <div class="flex items-center gap-2">
                        ...
                        <i :class="selectedTeamId === g.id ? 'pi pi-chevron-up' : 'pi pi-chevron-down'" 
                    ...
                    <div class="flex flex-wrap gap-1 mb-2">
                      <div v-for="p in g.players.slice(0, 5)" :key="p.id" class="relative group">
                        <img :src="getProfessionIconUrl(p.profession)" class="w-6 h-6 rounded-full border" 
                          :style="{ borderColor: groupColor(g.id) + '60' }"
                          :title="p.character_name || p.account" />
                        <div v-if="p.has_commander_tag" class="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 bg-yellow-500 rounded-full flex items-center justify-center">
                          <i class="pi pi-star-fill text-[6px] text-yellow-900" />
                        </div>
                      </div>
                      <div v-if="g.players.length > 5" class="w-6 h-6 rounded-full bg-neutral-bg flex items-center justify-center text-[10px] text-neutral-text-secondary">
                        +{{ g.players.length - 5 }}
                      </div>
                    </div>

                    <!-- 展开的小队详情 -->
                    <div v-show="selectedTeamId === g.id" class="pt-2 border-t border-neutral-border/30 space-y-2">
                  ...
                  <!-- 未分组玩家 -->
                  <div v-if="ungroupedPlayers.length > 0" class="card p-3 rounded-xl border border-dashed border-neutral-border/50">
                    <div class="flex items-center justify-between mb-2">
                      <div class="flex items-center gap-2">
                    ...
                    <div class="flex flex-wrap gap-1">
                      <div v-for="p in ungroupedPlayers.slice(0, 8)" :key="p.id" class="relative group">
                        <img :src="getProfessionIconUrl(p.profession)" class="w-6 h-6 rounded-full border border-neutral-border/50 cursor-pointer hover:border-primary/50 transition-colors"
                          @click="openPlayerDialog(p)" />
                        <div v-if="p.has_commander_tag" class="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 bg-yellow-500 rounded-full flex items-center justify-center">
                          <i class="pi pi-star-fill text-[6px] text-yellow-900" />
                        </div>
                      </div>
                      <div v-if="ungroupedPlayers.length > 8" class="w-6 h-6 rounded-full bg-neutral-bg flex items-center justify-center text-[10px] text-neutral-text-secondary">
            ...
            <!-- 敌方目标（简化显示） -->
            <div v-if="summary?.enemy_players?.length" class="card p-4 rounded-xl border-error/20 bg-error/5">
              <h4 class="text-sm font-semibold text-neutral-text mb-3 flex items-center gap-2">
              ...
              <div class="flex flex-wrap gap-2">
                <div v-for="p in summary.enemy_players.slice(0, 10)" :key="p.id" 
                  class="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-error/10 border border-error/20">
                  <img :src="getProfessionIconUrl(p.profession)" class="w-6 h-6 rounded-full border border-error/30" />
                ...
                <div v-if="(summary.enemy_players.length || 0) > 10" class="flex items-center px-3 py-1.5 text-xs text-neutral-text-secondary">
  ...
  <!-- 伤害构成详情弹框 -->
  <Dialog v-model:visible="showDamageDetailDialog" header="伤害构成详情" :style="{ width: '800px', maxWidth: '95vw' }" :modal="true" :draggable="false">
    <div class="space-y-6">
      <!-- 环形图 -->
      <div class="flex flex-col sm:flex-row items-center gap-6">
        <div class="relative w-48 h-48">
          <svg viewBox="0 0 100 100" class="w-full h-full -rotate-90 transform transition-all duration-500">
            <circle cx="50" cy="50" r="42" fill="none" stroke="var(--color-border)" stroke-width="12" />
            <circle cx="50" cy="50" r="42" fill="none" stroke="var(--color-primary)" stroke-width="12" 
              :stroke-dasharray="donut.pd" class="transition-all duration-700" />
            <circle cx="50" cy="50" r="42" fill="none" stroke="var(--color-success)" stroke-width="12" 
              :stroke-dasharray="donut.cd" :stroke-dashoffset="donut.co" class="transition-all duration-700" />
            <circle cx="50" cy="50" r="42" fill="none" stroke="var(--color-secondary)" stroke-width="12" 
              :stroke-dasharray="donut.bd" :stroke-dashoffset="donut.bo" class="transition-all duration-700" />
          </svg>
        ...
        <div class="flex-1 space-y-4">
          <div class="flex items-center justify-between p-4 rounded-xl bg-primary/10 border border-primary/20">
            ...
            <div class="text-right">
              <p class="text-xl font-bold text-primary">{{ fmtCompact(agg.total_power_damage) }}</p>
              <p class="text-xs text-neutral-text-secondary">{{ donut.p }}%</p>
            </div>
          </div>
          <div class="flex items-center justify-between p-4 rounded-xl bg-success/10 border border-success/20">
            ...
            <div class="text-right">
              <p class="text-xl font-bold text-success">{{ fmtCompact(agg.total_condi_damage) }}</p>
              <p class="text-xs text-neutral-text-secondary">{{ donut.c }}%</p>
            </div>
          </div>
          <div class="flex items-center justify-between p-4 rounded-xl bg-secondary/10 border border-secondary/20">
            ...
            <div class="text-right">
              <p class="text-xl font-bold text-secondary">{{ fmtCompact(agg.total_breakbar_damage) }}</p>
              <p class="text-xs text-neutral-text-secondary">{{ breakbarPct }}%</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <DataTable :value="topDpsPlayers" :paginator="true" :rows="10" class="w-full" scrollable>
          <Column field="rank" header="排名" style="width: 60px">
            <template #body="{ index }">
              <span class="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold" :class="rankClass(index)">{{ index + 1 }}</span>
            </template>
          </Column>
          <Column field="character_name" header="玩家" style="min-width: 140px">
            <template #body="{ data }">
              <div class="flex items-center gap-2">
                ...
                <div>
                  <p class="text-sm font-medium">{{ data.character_name || data.account }}</p>
                  <p class="text-xs text-neutral-text-secondary">{{ getProfessionName(data.profession) }}</p>
                </div>
              </div>
            </template>
          </Column>
          <Column field="damage" header="总伤害" style="min-width: 120px">
            <template #body="{ data }">
              <span class="text-sm font-bold text-primary">{{ fmtCompact(data.damage) }}</span>
            </template>
          </Column>
          <Column field="power_damage" header="直伤" style="min-width: 100px">
            <template #body="{ data }">
              <span class="text-sm text-primary/80">{{ fmtCompact(data.power_damage) }}</span>
            </template>
          </Column>
          <Column field="condi_damage" header="症状" style="min-width: 100px">
            <template #body="{ data }">
              <span class="text-sm text-success/80">{{ fmtCompact(data.condi_damage) }}</span>
            </template>
          </Column>
          <Column field="dps" header="DPS" style="min-width: 100px">
            <template #body="{ data }">
              <span class="text-sm font-semibold text-neutral-text">{{ fmtCompact(data.dps) }}</span>
            </template>
          </Column>
          <Column field="damage_percent" header="占比" style="min-width: 80px">
            <template #body="{ data }">
  ...
  <!-- 战斗属性详情弹框 -->
  <Dialog v-model:visible="showStatDetailDialog" :header="statDetailTitle" :style="{ width: '700px', maxWidth: '95vw' }" :modal="true" :draggable="false">
    <div class="space-y-4">
      <!-- 统计摘要 -->
      <div class="flex items-center gap-4 p-3 rounded-xl bg-neutral-bg-secondary border border-neutral-border/50">
        ...
        <div v-if="statDetailList.length > 0" class="flex items-center gap-2 ml-auto">
      ...
      <!-- 玩家列表 -->
      <DataTable :value="statDetailList" :paginator="true" :rows="10" class="w-full" scrollable scrollHeight="400px">
        <Column field="rank" header="排名" style="width: 60px">
          <template #body="{ index }">
            <span class="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold" :class="rankClass(index)">{{ index + 1 }}</span>
          </template>
        </Column>
        <Column field="character_name" header="玩家" style="min-width: 160px">
          <template #body="{ data }">
            <div class="flex items-center gap-2 cursor-pointer hover:text-primary transition-colors" @click="openPlayerDialog(data)">
              <img :src="getProfessionIconUrl(data.profession)" class="w-6 h-6 rounded-full" />
              <div>
                <p class="text-sm font-medium">{{ data.character_name || data.account }}</p>
                <p class="text-xs text-neutral-text-secondary">{{ getProfessionName(data.profession) }}</p>
        ...
        <Column field="account" header="账号" style="min-width: 120px">
          <template #body="{ data }">
            <span class="text-xs text-neutral-text-secondary">{{ data.account }}</span>
          </template>
        </Column>
        <Column :field="currentStatType" header="数值" style="min-width: 120px">
          <template #body="{ data }">
            <span class="text-sm font-semibold" :class="getStatValueClass(currentStatType, data)">{{ getStatValue(data, currentStatType) }}</span>
          </template>
        </Column>
      </DataTable>
    </div>
  ...
  <!-- 玩家详情弹框 -->
  <Dialog v-model:visible="dialogVisible" :header="selectedPlayer ? (selectedPlayer.character_name || selectedPlayer.account) : '玩家详情'" :style="{ width: '900px', maxWidth: '95vw' }" :modal="true" :draggable="false">
    <div v-if="selectedPlayer" class="space-y-5">
      <!-- 玩家信息头 -->
      <div class="flex items-center gap-4 pb-4 border-b border-neutral-border">
        <img :src="getProfessionIconUrl(selectedPlayer.profession)" class="w-12 h-12 rounded-full" />
        <div>
          <p class="text-lg font-bold text-neutral-text">{{ selectedPlayer.character_name || selectedPlayer.account }}</p>
          <p v-if="selectedPlayer.account && selectedPlayer.character_name" class="text-sm text-neutral-text-secondary">{{ selectedPlayer.account }}</p>
          <p class="text-sm text-neutral-text-secondary">{{ getProfessionName(selectedPlayer.profession) }}</p>
        </div>
        <div class="ml-auto flex gap-3 text-sm">
          <div class="text-center"><p class="font-bold text-primary">{{ fmtCompact(selectedPlayer.damage) }}</p><p class="text-xs text-neutral-text-secondary">总伤害</p></div>
          <div class="text-center"><p class="font-bold text-primary">{{ fmtCompact(selectedPlayer.dps) }}</p><p class="text-xs text-neutral-text-secondary">DPS</p></div>
          <div class="text-center"><p class="font-bold text-status-success">{{ selectedPlayer.score_grade || '-' }}</p><p class="text-xs text-neutral-text-secondary">评分</p></div>
        </div>
      </div>

      <!-- 加载状态 -->
      <div v-if="rotationLoading" class="flex items-center justify-center py-8">
      ...
      <template v-else-if="playerRotation">
        <!-- 武器配置 -->
        <div v-if="playerRotation.weapons?.length" class="pb-4 border-b border-neutral-border">
          ...
          <div class="flex items-center gap-3 flex-wrap">
            <div v-for="(w, idx) in playerRotation.weapons.filter((x: string) => x && x !== 'Unknown').slice(0, 4)" :key="idx"
        ...
        <!-- 食物/扳手 -->
        <div v-if="playerRotation.consumables?.food?.length || playerRotation.consumables?.utility?.length" class="pb-4 border-b border-neutral-border">
          <h4 class="text-sm font-semibold text-neutral-text mb-2 flex items-center gap-2">
            <i class="pi pi-sparkles text-status-success" /> 食物 / 扳手
          </h4>
          <div class="flex flex-wrap gap-2">
            <div v-for="(f, idx) in playerRotation.consumables.food" :key="`food-${idx}`" class="flex items-center gap-2 px-2 py-1 rounded bg-status-success/10 text-xs text-neutral-text">
              <img v-if="f.icon" :src="f.icon" class="w-5 h-5 rounded" />
              <span>{{ f.name || '未知食物' }}</span>
            </div>
            <div v-for="(u, idx) in playerRotation.consumables.utility" :key="`util-${idx}`" class="flex items-center gap-2 px-2 py-1 rounded bg-primary/10 text-xs text-neutral-text">
              <img v-if="u.icon" :src="u.icon" class="w-5 h-5 rounded" />
        ...
        <!-- 无详细数据时统一提示 -->
        <div v-if="!hasPlayerDetailData" class="text-neutral-text-secondary text-sm text-center py-8">
          <i class="pi pi-info-circle text-2xl mb-2 block" />
          <p>暂无详细战斗数据</p>
          <p class="text-xs mt-1">当前解析器暂未提供技能循环、武器配置等详细信息</p>
        </div>
        ...
        <div v-else class="grid grid-cols-1 lg:grid-cols-2 gap-5">
          ...
          <div v-if="sortedSkillCasts.length > 0">
            ...
            <div class="max-h-[400px] overflow-auto space-y-1">
              <div v-for="s in sortedSkillCasts" :key="s.skillId" class="flex items-center gap-3 p-2 rounded hover:bg-neutral-bg/50">
                <img v-if="s.icon" :src="s.icon" class="w-8 h-8 rounded" />
                <div v-else class="w-8 h-8 rounded bg-neutral-bg flex items-center justify-center text-xs text-neutral-text-secondary">?</div>
          ...
          <div v-if="rotationEvents.length > 0">
            ...
            <div class="max-h-[400px] overflow-auto space-y-1">
              <div v-for="(e, idx) in rotationEvents" :key="idx" class="flex items-center gap-3 p-2 rounded hover:bg-neutral-bg/50">
                <img v-if="e.icon" :src="e.icon" class="w-8 h-8 rounded" />
                <div v-else class="w-8 h-8 rounded bg-neutral-bg flex items-center justify-center text-xs text-neutral-text-secondary">?</div>
                <div class="flex-1 min-w-0">
                  <p class="text-sm text-neutral-text truncate">{{ e.name }}</p>
                  <p class="text-[10px] text-neutral-text-secondary">{{ e.duration > 0 ? e.duration + 'ms' : '瞬发' }}</p>
                </div>
      ...
      <div v-else class="text-neutral-text-secondary text-sm text-center py-8">
          <i class="pi pi-info-circle text-2xl mb-2 block" />
          <p>该日志未生成技能循环数据</p>
          <p class="text-xs mt-1">请重新解析日志以获取技能详情</p>
        </div>
    </div>
  </Dialog>
