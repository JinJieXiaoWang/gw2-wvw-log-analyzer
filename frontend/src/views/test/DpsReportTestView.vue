<template>
  <div class="space-y-6">
    <!-- 页面标题 -->
    <div>
      <h1 class="text-2xl font-bold text-neutral-text">dps.report API 测试</h1>
    ...
    <div class="card p-6">
      ...
      >
        ...
        <p class="text-neutral-text font-medium">点击或拖拽上传 zevtc 文件</p>
        <p class="text-neutral-text-secondary text-xs mt-1">支持 .zevtc / .evtc / .evtc.zip</p>
        <input ref="fileInput" type="file" accept=".zevtc,.evtc,.evtc.zip" class="hidden" @change="onFileChange" />
      </div>

      <div v-if="selectedFile" class="mt-4 flex items-center gap-3 p-3 rounded bg-neutral-bg">
        <i class="pi pi-file text-primary" />
        <div class="flex-1 min-w-0">
          <p class="text-sm text-neutral-text truncate">{{ selectedFile.name }}</p>
          <p class="text-xs text-neutral-text-secondary">{{ fmtSize(selectedFile.size) }}</p>
        </div>
        <Button label="开始测试" icon="pi pi-play" :loading="loading" @click="startTest" />
      </div>
    </div>
    ...
    <div v-if="loading || result" class="card p-6">
      ...
      <div v-else-if="result" class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div class="text-center p-3 rounded bg-neutral-bg">
          <p class="text-xl font-bold text-primary">{{ result.upload_time_ms }}ms</p>
          <p class="text-xs text-neutral-text-secondary">上传耗时</p>
        </div>
        <div class="text-center p-3 rounded bg-neutral-bg">
          <p class="text-xl font-bold text-primary">{{ result.json_fetch_time_ms }}ms</p>
          <p class="text-xs text-neutral-text-secondary">JSON拉取</p>
        </div>
        <div class="text-center p-3 rounded bg-neutral-bg">
          <p class="text-xl font-bold text-primary">{{ result.total_time_ms }}ms</p>
          <p class="text-xs text-neutral-text-secondary">总耗时</p>
        </div>
        <div class="text-center p-3 rounded bg-neutral-bg">
          <p class="text-xl font-bold text-primary">{{ result.ei_version || '-' }}</p>
          <p class="text-xs text-neutral-text-secondary">EI 版本</p>
        </div>
      </div>
    </div>
    ...
    <div v-if="result" class="card p-6">
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
        <div class="text-center p-3 rounded bg-neutral-bg">
          <p class="text-xl font-bold text-primary">{{ result.player_count }}</p>
          <p class="text-xs text-neutral-text-secondary">玩家数</p>
        </div>
        <div class="text-center p-3 rounded bg-neutral-bg">
          <p class="text-xl font-bold text-primary">{{ result.target_count }}</p>
          <p class="text-xs text-neutral-text-secondary">目标数</p>
        </div>
        <div class="text-center p-3 rounded bg-neutral-bg">
          <p class="text-xl font-bold text-primary">{{ result.skill_map_count }}</p>
          <p class="text-xs text-neutral-text-secondary">技能映射</p>
        </div>
        <div class="text-center p-3 rounded bg-neutral-bg">
          <a :href="result.permalink" target="_blank" class="text-sm text-primary hover:underline">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div class="flex items-center gap-2 p-3 rounded" :class="result.has_rotation ? 'bg-status-success/10' : 'bg-status-error/10'">
          ...
          <span class="text-sm" :class="result.has_rotation ? 'text-status-success' : 'text-status-error'">
        ...
        <div class="flex items-center gap-2 p-3 rounded" :class="result.has_weapons ? 'bg-status-success/10' : 'bg-status-error/10'">
          ...
          <span class="text-sm" :class="result.has_weapons ? 'text-status-success' : 'text-status-error'">
        ...
        <div class="flex items-center gap-2 p-3 rounded" :class="result.has_death_recap ? 'bg-status-success/10' : 'bg-status-error/10'">
          ...
          <span class="text-sm" :class="result.has_death_recap ? 'text-status-success' : 'text-status-error'">
    ...
    <div v-if="result?.raw_json" class="card p-6 space-y-4">
      ...
      <div v-if="result.has_rotation">
        <h4 class="text-xs font-semibold text-neutral-text-secondary mb-2 uppercase tracking-wide">技能循环 (前3条)</h4>
      ...
      <div v-if="result.has_weapons">
        <h4 class="text-xs font-semibold text-neutral-text-secondary mb-2 uppercase tracking-wide">武器配置</h4>
        <div class="bg-neutral-bg rounded p-3 overflow-auto max-h-[200px]">
      ...
      <div v-if="result.skill_map_count > 0">
        <h4 class="text-xs font-semibold text-neutral-text-secondary mb-2 uppercase tracking-wide">SkillMap 前10个key</h4>
        <div class="bg-neutral-bg rounded p-3 overflow-auto max-h-[200px]">
