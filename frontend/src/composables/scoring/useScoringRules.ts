/**
 * useScoringRules - 评分规则配置业务逻辑 composable（统一版本）
...
import { ROLE_ICON_MAP, ROLE_GRADIENT_MAP, DEFAULT_ROLE_TYPES, DIMENSION_ICONS, DIMENSION_COLORS } from '@/config/scoringConstants'

export interface UseScoringRulesOptions {
  /** 是否启用职业特定规则 */
  enableProfessionRules?: boolean
  /** 是否启用历史重算 */
...
export function useScoringRules(options: UseScoringRulesOptions = {}) {
  const {
  ...
  async function fetchProfessions() {
    if (!enableProfessionRules) return
    try {
  ...
  function switchScope(scope: 'generic' | 'profession') {
    ...
    if (enableProfessionRules && scope === 'profession' && cascadeProfessions.value.length === 0) fetchProfessions()
    syncEditableRules()
  }
  ...
  return {
    ...
    roleTypes, activeRole, currentRules, originalRules, editableRules, changedRoles,
    ...
    // Actions
    fetchRoleTypes, fetchRules, fetchDimensions, fetchProfessions, fetchVersions,
    syncEditableRules,
    switchRole, switchScope, markChanged, moveUp, moveDown, removeRule, addRule,
    saveChanges, resetChanges, confirmReset, onBaseProfessionChange, onProfessionChange,
    confirmRecalculate, startRecalculation, pollRecalcStatus, confirmDeleteProfessionRules,
  }
}
