...
const roleGradientMap: Record<string, string> = { dps: Colors.palette.salmon, support: Colors.palette.skyBlue, tank: Colors.primary.DEFAULT, condition: Colors.palette.orangeRed, healing: Colors.palette.aqua, control: Colors.palette.indigo, utility: Colors.palette.sky }
export function useScoringRules() {
  ...
  const roleColors = computed(() => {
    ...
    for (const r of roleTypes.value) map[r.type] = r.color || Colors.palette.gray
    return map
  })
  const roleGradients = computed(() => {
    const map: Record<string, string> = {}
    for (const r of roleTypes.value) map[r.type] = roleGradientMap[r.type] || r.color || Colors.palette.gray
    return map
  })
  ...
  const dimensionColors: Record<string, string> = {
    damage: Colors.palette.roseBright, healing: Colors.palette.mint, protection: Colors.primary.DEFAULT,
    crowd_control: Colors.palette.violetBright, support: Colors.palette.orangeBright, survival: Colors.palette.skyBlue,
    objective: Colors.palette.materialGreen, downstacks: Colors.palette.deepOrange,
  }
  function getDimensionIcon(key: string): string { return dimensionIcons[key] || 'pi pi-circle' }
  function getDimensionColor(key: string): string { return dimensionColors[key] || Colors.palette.grayMedium }
    ...
    try {
      ...
      if (data && data.length > 0) {
        roleTypes.value = data.map((r: any) => ({
          ...
          icon: roleIconMap[r.type] || 'pi pi-star', color: r.color || Colors.palette.gray,
        }))
      ...
      roleTypes.value = [
        { type: 'dps', label: '输出', description: '以伤害输出为主要职责', icon: 'pi pi-bolt', color: Colors.palette.roseBright },
        { type: 'support', label: '辅助', description: '以治疗和增益为主要职责', icon: 'pi pi-heart', color: Colors.palette.mint },
        { type: 'tank', label: '承伤', description: '以吸收伤害和控制为主要职责', icon: 'pi pi-shield', color: Colors.palette.violetBright },
      ]
