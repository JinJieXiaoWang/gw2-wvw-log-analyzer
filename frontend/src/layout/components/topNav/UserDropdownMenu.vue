<template>
  ...
  >
    <div
      v-if="visible"
        class="user-dropdown absolute top-full mt-2 right-0 w-64 bg-neutral-card border
               border-neutral-border rounded-xl shadow-2xl shadow-neutral-shadow/50
    ...
    >
      <div v-if="isAuthenticated">
        <div
          ...
          <div class="flex items-center gap-3">
            <div
              class="w-12 h-12 bg-gradient-to-br from-primary to-secondary rounded-xl flex
        ...
        <div class="p-2">
          <router-link
            v-if="canWrite"
            to="/settings"
              class="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-neutral-bg
        ...
        <div class="px-4 py-3 border-t border-neutral-border bg-neutral-bg/50">
          <button
            class="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-gradient-to-r
                   from-status-error/10 to-status-error/5 hover:from-status-error/20
                   hover:to-status-error/10 text-status-error rounded-lg transition-all group
                   logout-btn"
            @click="emit('logout')"
          >
      ...
      <div v-else>
        <div class="px-4 py-6 text-center border-b border-neutral-border">
          <div
            class="w-16 h-16 bg-neutral-bg rounded-full flex items-center justify-center
                   mx-auto mb-3">
          ...
          <router-link
            to="/login"
              class="inline-flex items-center gap-2 px-6 py-2.5 bg-gradient-to-r from-primary
                     to-secondary text-white rounded-lg hover:opacity-90 transition-opacity
