...
async def test_dps_report(file: UploadFile = File(...)):
    """
    上传 zevtc 文件到 dps.report，测试 EI 解析速度和数据完整性。
    ...
    if not file.filename or not file.filename.endswith(
        ...
        return ApiResponse(
            success=False,
            message="仅支持 .zevtc / .evtc / .evtc.zip 文件",
            code=400,
            data=None,
        )
    ...
    try:
        ...
        logger.info(
            f"[dps.report测试] 文件大小: {len(content)} bytes, 文件名: {file.filename}"
        )

        # Step 1: 上传到 dps.report
        upload_start = time.time()
        files = {"file": (file.filename, content, "application/octet-stream")}
        upload_resp = requests.post(
            f"{DPS_REPORT_UPLOAD_URL}?json=1&generator=ei&detailedwvw=true",
            files=files,
