...
from app.schemas.game_data.profession import (
    ProfessionCreate,
    ProfessionUpdate,
...
async def update_profession_role(
    ...
    if success:
        return ApiResponse.success_response(
            message=f"职业 {profession_key} 的默认角色定位已更新?{role_update.role_key}",
        )
    else:
        return ApiResponse.fail_response(message=f"更新失败，职?{profession_key} 不存?)
...
async def update_spec_role(
    ...
    if success:
        return ApiResponse.success_response(
            message=f"精英特长 {spec_key} 的默认角色定位已更新?{role_update.role_key}",
        )
    else:
        return ApiResponse.fail_response(message=f"更新失败，精英特?{spec_key} 不存?)


@router.post("/profession", response_model=ApiResponse, summary="创建新职?)
...
async def delete_profession(
    ...
    if success:
        return ApiResponse.success_response(message=f"职业 {profession_key} 已删?)
    else:
        return ApiResponse.fail_response(message=f"删除失败，职?{profession_key} 不存?)


@router.post("/elite-spec", response_model=ApiResponse, summary="创建新精英特?)
...
async def delete_elite_spec(
    ...
    if success:
        return ApiResponse.success_response(message=f"精英特长 {spec_key} 已删?)
    else:
        return ApiResponse.fail_response(message=f"删除失败，精英特?{spec_key} 不存?)
...
async def update_role_type(
    ...
    if role:
        ...
        return ApiResponse.fail_response(message=f"更新失败，角色定义{role_key} 不存?)
...
async def delete_role_type(
    ...
    if success:
        return ApiResponse.success_response(message=f"角色定位 {role_key} 已删?)
    else:
        return ApiResponse.fail_response(message=f"删除失败，角色定义{role_key} 不存?)
