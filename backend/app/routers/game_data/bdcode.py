...
from app.schemas.game_data.bdcode import (
    BDCodeBatchItem,
    BDCodeBatchRequest,
    BDCodeBatchResponse,
    BDCodeParseRequest,
...
from app.utils.error.exceptions import BadRequestException, InternalServerErrorException
from app.utils.logger import logger

router = APIRouter(prefix="/bdcode", tags=["BDCode解析"])

# 常量定义
BATCH_PARSE_MAX_COUNT = 50
HEALTH_CHECK_TEST_BD_CODE = "[&DQg1KTIlIjbBEigPQAGBAIEAQAHxEnUBAxOVAAAAAAAAAAAAAAAAAAAAAAA=]"
...
async def parse_bdcode(request: BDCodeParseRequest):
    ...
    try:
        ...
        raise InternalServerErrorException(detail=f"解析失败: {str(e)}")
...
async def validate_bdcode(request: BDCodeValidationRequest):
    ...
    try:
        ...
        raise InternalServerErrorException(detail=f"验证失败: {str(e)}")
...
async def parse_batch(request: BDCodeBatchRequest):
    ...
    批量解析BDCode（最?0个）
    """
    logger.info(f"批量解析BDCode: {len(request.bd_codes)}?)

    if len(request.bd_codes) > BATCH_PARSE_MAX_COUNT:
        raise BadRequestException(detail=f"单次最多解析{BATCH_PARSE_MAX_COUNT}个BDCode")
    ...
    try:
        ...
        raise InternalServerErrorException(detail=f"批量解析失败: {str(e)}")
...
async def get_bdcode_stats():
    ...
    try:
        ...
        raise InternalServerErrorException(detail=f"获取统计失败: {str(e)}")


# ==================== 健康检?====================
@router.get("/health", summary="BDCode解析服务健康检?)
async def bdcode_health():
    ...
    try:
        service = get_bdcode_service()
        test_bd = HEALTH_CHECK_TEST_BD_CODE
        ...
        return ApiResponse.fail_response(
            message="BDCode解析服务运行异常",
            data={"status": "unhealthy", "service": "bdcode_parser", "error": str(e)},
        )
