...
@router.get("", response_model=ApiResponse, summary="获取所有职业列?)
...
@router.get("/elite-specs", response_model=ApiResponse, summary="获取所有精英特?)
...
@router.get("/elite-specs/{profession_key}", response_model=ApiResponse, summary="获取职业的精英特?)
async def get_profession_elite_specs(
    ...
    return ApiResponse.success_response(
        message=f"获取 {profession_key} 的精英特长成?,
        data={
...
async def get_profession_detail(
    ...
    if not profession:
        return ApiResponse.fail_response(message=f"职业 {profession_key} 不存?)
    return ApiResponse.success_response(
        message="获取职业详情成功",
        data=profession,
    )
...
async def get_elite_spec_detail(
    ...
    if not spec:
        return ApiResponse.fail_response(message=f"精英特长 {spec_key} 不存?)
    return ApiResponse.success_response(
        message="获取精英特长详情成功",
        data=spec,
    )
...
@router.get("/name/{profession_key}", response_model=ApiResponse, summary="获取职业中文?)
