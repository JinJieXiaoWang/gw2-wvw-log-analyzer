...
from app.schemas.game_data.build import BuildCreate, BuildListResponse, BuildResponse, BuildUpdate
...
from app.services.game_data.build_service import (
    create_build,
    delete_build,
    get_build_by_id,
    list_builds,
...
router = APIRouter(prefix="/builds", tags=["Build图书?])
...
async def get_builds(
    ...
    sub_role: Optional[str] = Query(None, description="子角色过?),
...
async def get_build(build_id: int, db: Session = Depends(get_db)):
    ...
    if not build:
        raise NotFoundException(f"Build ID {build_id} 不存?)
    return ApiResponse.success_response(
        message="获取Build成功",
