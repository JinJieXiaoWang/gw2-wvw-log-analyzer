    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
                        ...
                        if hasattr(orig_exc, "args") and len(orig_exc.args) >= 2:
                            if orig_exc.args[0] == MYSQL_LOCK_WAIT_TIMEOUT:
                                if retries < max_retries:
                                    logger.warning(
                                        f"数据库锁等待超时，正在重试 ({retries + 1}/{max_retries})，延迟 {delay:.2f}s"
                                    )
                                    time.sleep(delay)
                                    retries += 1
                                    ...
                                    logger.error(
                                        f"数据库锁等待超时，已达到最大重试次数 ({max_retries})"
                                    )
            ...
            raise OperationalError(
                f"数据库操作失败：锁等待超时，已重试 {max_retries} 次",
                params=None,
                orig=None,
            )
...
def execute_with_retry(db_session, query_func, *args, max_retries: int = 3):
    ...
    Args:
        db_session: 数据库会话
        query_func: 查询函数，接收 db_session 和 *args
        *args: 传递给 query_func 的参数
        max_retries: 最大重试次数
            ...
            if hasattr(e, "args") and len(e.args) > 0:
                ...
                if hasattr(orig_exc, "args") and len(orig_exc.args) >= 2:
                    if orig_exc.args[0] == MYSQL_LOCK_WAIT_TIMEOUT:
                        if retries < max_retries:
                            logger.warning(
                                f"数据库锁等待超时，正在重试 ({retries + 1}/{max_retries})"
                            )
                            time.sleep(delay)
                            retries += 1
                            delay *= 2
    ...
    raise OperationalError(
        f"数据库操作失败：锁等待超时，已重试 {max_retries} 次", params=None, orig=None
    )
