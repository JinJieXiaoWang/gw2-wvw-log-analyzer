...
class PlayerScoreItem(BaseModel):
    ...
    member_id: int = Field(..., description="成员 ID（关联 members 表）")
    account: str = Field(..., description="玩家账号名")
...
class FightScoreResult(BaseModel):
    ...
    fight_id: int = Field(..., description="战斗记录 ID（关联 fights 表）")
