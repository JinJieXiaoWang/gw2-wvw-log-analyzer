...
说明：
  EI (_logData + _graphData) 的数据结构极其复杂且版本间可能变化，
  因此本模块采用"顶层结构化 + 深层宽松化"的策略：
  - 顶层字段（如元数据、定义表）使用显式 Pydantic 字段
  - 深层嵌套结构（如 player details, phase stats）使用 Dict[str, Any] / List[Any]
  这样既保证 API 文档可读性，又避免因 EI 版本变化导致解析失败。
"""
