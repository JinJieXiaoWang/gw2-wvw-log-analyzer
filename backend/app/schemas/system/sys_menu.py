...
class SysMenuBase(BaseModel):
    ...
    is_frame: int = Field(1, ge=0, le=1, description="是否为外链（0?1否）")
    is_cache: int = Field(0, ge=0, le=1, description="是否缓存?缓存 1不缓存）")
...
class SysMenuCreate(SysMenuBase):
    """创建菜单请求模型"""
    create_by: str = Field("", max_length=64, description="创建?)


class SysMenuUpdate(SysMenuBase):
    """更新菜单请求模型"""
    update_by: str = Field("", max_length=64, description="更新?)
...
class SysMenuResponse(BaseModel):
    ...
    is_frame: int = Field(..., description="是否为外?)
    is_cache: int = Field(..., description="是否缓存")
    menu_type: str = Field(..., description="菜单类型")
    visible: str = Field(..., description="显示状?)
    status: str = Field(..., description="菜单状?)
    perms: Optional[str] = Field(None, description="权限标识")
    icon: str = Field(..., description="菜单图标")
    create_by: str = Field(..., description="创建?)
    create_time: Optional[datetime] = Field(None, description="创建时间")
    update_by: str = Field(..., description="更新?)
...
class SysMenuTreeResponse(SysMenuResponse):
    """菜单树形响应模型"""
    children: Optional[List["SysMenuTreeResponse"]] = Field([], description="子菜单列?)
...
class MenuQueryRequest(BaseModel):
    ...
    status: Optional[str] = Field(None, description="菜单状?)
    visible: Optional[str] = Field(None, description="显示状?)
...
class MenuPermissionResponse(BaseModel):
    """菜单权限检查响应模型""
    has_access: bool = Field(..., description="是否有权限访?)
    menu_id: int = Field(..., description="菜单ID")
    menu_name: str = Field(..., description="菜单名称")
    missing_perms: List[str] = Field([], description="缺失的权限列?)
