...
class DictDataBase(BaseModel):
    ...
    css_class: Optional[str] = Field(None, description="CSS样式类/颜色值")
    list_class: Optional[str] = Field(None, description="列表样式类")
...
class DictDataUpdate(BaseModel):
    ...
    list_class: Optional[str] = Field(None, description="列表样式类")
    is_default: Optional[int] = Field(None, description="是否默认值")
...
class DictDataResponse(DictDataBase):
    # 功能：字典项响应模型
    dict_code: int = Field(..., description="字典项编码")
...
class DictOption(BaseModel):
    ...
    css_class: Optional[str] = Field(None, description="CSS样式类/颜色值")
    is_default: int = Field(0, description="是否默认值")
