...
class BDCodeValidationRequest(BaseModel):
    ...
    bd_code: str = Field(..., description="BDCode字符串")
...
class SpecializationInfoResponse(BaseModel):
    ...
    name: str = Field("", description="专精线名称")
    name_cn: Optional[str] = Field(None, description="中文名称")
    icon: Optional[str] = Field(None, description="图标URL")
    is_elite: bool = Field(False, description="是否为精英专精")
    selected_traits: List[int] = Field([0, 0, 0], description="选中的特性位置")
    traits: List[TraitInfoResponse] = Field([], description="特性列表")
...
class BuildInfoResponse(BaseModel):
    ...
    skills: Dict[str, Any] = Field({}, description="技能列表（heal, utility, elite）")
...
class BDCodeStatsResponse(BaseModel):
    ...
    skills_count: int = Field(..., description="技能数据数量")
    specializations_count: int = Field(..., description="专精线数据数量")
    traits_count: int = Field(..., description="特性数据数量")
