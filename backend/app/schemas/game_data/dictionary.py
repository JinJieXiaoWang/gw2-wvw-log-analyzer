...
class DictTypeBase(BaseModel):
    ...
    status: int = Field(0, description="状态：0-启用?-禁用")
...
class DictTypeUpdate(BaseModel):
    # 功能：字典类型更新模型    model_config = ConfigDict(from_attributes=True)

    dict_name: Optional[str] = Field(None, description="字典类型名称")
    status: Optional[int] = Field(None, description="状态：0-启用?-禁用")
    sort_order: Optional[int] = Field(None, description="排序顺序")
    remark: Optional[str] = Field(None, description="备注说明")


class DictTypeResponse(DictTypeBase):
    # 功能：字典类型响应模型    dict_id: int = Field(..., description="字典类型ID")
    is_system: int = Field(0, description="是否系统预置?-否，1-?)
...
class DictDataBase(BaseModel):
    ...
    dict_type: str = Field(..., description="所属字典类)
    ...
    is_default: int = Field(0, description="是否默认值：0-否，1-?)
    status: int = Field(0, description="状态：0-启用?-禁用")
    remark: Optional[str] = Field(None, description="备注说明")
...
class DictDataUpdate(BaseModel):
    ...
    dict_value: Optional[str] = Field(None, description="字典?)
    ...
    status: Optional[int] = Field(None, description="状?)
    remark: Optional[str] = Field(None, description="备注说明")


class DictDataResponse(DictDataBase):
    # 功能：字典项响应模型
    dict_code: int = Field(..., description="字典项编?)
...
class DictOption(BaseModel):
    ...
    value: Any = Field(..., description="字典?)
