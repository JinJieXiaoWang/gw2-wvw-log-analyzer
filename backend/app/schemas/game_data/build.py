...
class BuildBase(BaseModel):
    ...
    role: str = Field(..., pattern=r"^(dps|support)$", description="主角?)
    ...
    attr_requirements: List[str] = Field(default_factory=list, description="属性要?)
    bd_code: str = Field(
        ...,
        pattern=r"^\[&[A-Za-z0-9+/=]+\]$",
    ...
    author: str = Field(default="", max_length=50, description="作?)
