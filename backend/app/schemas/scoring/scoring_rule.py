...
class ScoringRuleBase(BaseModel):
    ...
    min_value: Optional[float] = Field(None, description="最小值阈?)
    max_value: Optional[float] = Field(None, description="最大值上?)
...
class ScoringRulePresetBase(BaseModel):
    ...
    is_default: bool = Field(False, description="是否为系统默?)
    is_active: bool = Field(True, description="是否启用")
...
class RoleScoringConfig(BaseModel):
    """角色类型评分配置（用于前端展示）"""
    role_type: str = Field(..., description="角色类型")
    role_label: str = Field(..., description="角色类型中文?)
