...
class ScoreBreakdown(BaseModel):
    ...
    quickness: float = Field(0.0, ge=0, le=100, description="急速维度评?)
    ...
    breakbar: float = Field(0.0, ge=0, le=100, description="破蔑视维度评?)
...
class PlayerScoreItem(BaseModel):
    ...
    total_score: float = Field(..., ge=0, le=100, description="综合评分总分?-100?)
    grade: str = Field(..., description="评分等级（S/A/B/C/D/F?)
    grade_label: str = Field(..., description="评分等级中文标签（如 S级、A级）")
    breakdown: ScoreBreakdown = Field(..., description="各维度评分明?)
