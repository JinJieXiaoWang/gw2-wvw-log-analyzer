...
class PlayerScoreService:
    ...
    def calculate_single_score(
        ...
        Args:
            db: 数据库会话
            stat: FightStats 记录
            max_values: 同场最大值（可选，不传则使用 stat 自身值作为 max）
            role_type: 强制指定角色类型（None 则按职业自动判断）
            profession: 强制指定职业（None 则使用 stat.profession）
    ...
    def attach_scores_to_stats(
        ...
        """为一组 FightStats 附加评分字段（列表查询用）

        优化：按 fight_id 分组，同一场战斗共享 max_values。
