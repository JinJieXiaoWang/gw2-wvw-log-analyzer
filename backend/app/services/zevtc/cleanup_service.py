...
def cleanup_file(db: Session, log_id: int) -> Dict[str, Any]:
    ...
    功能：删除已解析日志的源文件
    参数：log_id - 日志ID
    返回：成功返?{"success": True}，失败返?{"success": False, "error": 错误信息}

    说明?
    - 用户在确认解析数据正确后，可调用此方法删除源文件
    - 如果解析数据有误，用户可以保留源文件重新解析
    - 文件删除后，log.file_path 将被设置?None
    """
    try:
        log = db.query(Log).filter(Log.id == log_id).first()
        if not log:
            return {"success": False, "error": f"日志不存? {log_id}"}

        file_path = log.file_path
        if not file_path:
            return {"success": False, "error": "文件路径为空，可能已被删?}
        ...
        return {"success": True, "message": f"文件已删? {file_path}"}
    ...
    try:
        for log_id in log_ids:
            try:
                log = db.query(Log).filter(Log.id == log_id).first()
                if not log:
                    errors.append(f"log_id={log_id}: 日志不存?)
                    failed_count += 1
                    continue
...
def _delete_file(file_path: str):
    ...
    try:
        os.remove(file_path)
        logger.info(f"已删除解析文? {file_path}")
    except Exception as e:
