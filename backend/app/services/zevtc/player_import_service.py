...
def insert_players(db: Session, fight_id: int, players: List[Dict[str, Any]]):
    ...
    - 更新 AccountCharacter 映射，支持同一 account 多个角色
    【v4.0 变更】删除导入时评分计算，只保存原始数据。
    评分移至查询阶段（PlayerScoreService），规则更新立即生效。
    """
    today = date.today()

    # 批量预查询：收集所有需要的 account 和 account-character 对
    needed_accounts, needed_pairs = collect_needed_accounts_and_pairs(players)

    # 一次性查询所有已存在的 AccountCharacter 和 Member 记录
    ...
    new_acs, member_map = process_members_and_characters(
