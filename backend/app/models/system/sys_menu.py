...
class SysMenu(Base):
    ...
    is_frame = Column(INTEGER, default=MenuYesNo.YES, comment="是否为外链（0否 1是）")

    is_cache = Column(INTEGER, default=MenuYesNo.NO, comment="是否缓存（0不缓存 1缓存）")

    menu_type = Column(CHAR(1), default="", comment="菜单类型（M目录, C菜单, F按钮）")

    visible = Column(CHAR(1), default=MenuVisible.SHOW.value, comment="菜单状态（0显示 1隐藏）")

    status = Column(CHAR(1), default=MenuStatus.NORMAL.value, comment="菜单状态（0正常 1停用）")
