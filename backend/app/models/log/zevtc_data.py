...
# 依赖说明：SQLAlchemy
# 说明：
#   原evtc_header/agent/skill/event/player_instance/combat_meta/event_per_second
#   等7个原始数据模型已随ZEVTC 原始数据体系废弃而删除。
#   当前仅保留EI 同步相关模型（ei_player, ei_target, ei_skill_map, ei_phase）。
...
class EiPlayer(Base):
    ...
    account = Column(String(100), comment="玩家账户")
    ...
    has_commander_tag = Column(
        SmallInteger, default=0, comment="是否有指挥官标记(1=是，0=否)"
    )
...
class EiTarget(Base):
    ...
    enemy_player = Column(
        SmallInteger, default=0, comment="是否为敌方玩家（1=是，0=否）"
    )
...
class EiSkillMap(Base):
    ...
    auto_attack = Column(
        SmallInteger, default=0, comment="是否为自动攻击（1=是，0=否）"
    )
    ...
    is_instant_cast = Column(
        SmallInteger, default=0, comment="是否为瞬发（1=是，0=否）"
    )
    is_trait_proc = Column(
        SmallInteger, default=0, comment="是否为特性触发（1=是，0=否）"
    )
...
class EiPhase(Base):
    ...
    breakbar_phase = Column(
        SmallInteger, default=0, comment="是否为破蔑视阶段(1=是，0=否)"
    )
