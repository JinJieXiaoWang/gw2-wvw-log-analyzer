...
from sqlalchemy import Column, Integer, String, Float, Boolean, DateTime, Text, UniqueConstraint, func
...
class ScoringRule(Base):
    ...
    min_value = Column(
        Float,
        nullable=True,
        comment="最小值阈值，低于此值计0分",
    )
    max_value = Column(
        Float,
        nullable=True,
        comment="最大值上限，用于归一化分母",
    )
    is_active = Column(
        Boolean,
        default=True,
...
class ScoringRulePreset(Base):
    ...
    __table_args__ = {"comment": "评分规则预设方案表"}
