...
class SysDictType(Base):
    ...
    is_system = Column(
        Integer, nullable=False, default=0, comment="是否系统预置0-否，1-是"
    )
...
class SysDictData(Base):
    ...
    dict_code = Column(
        Integer, primary_key=True, index=True, autoincrement=True, comment="字典项编码"
    )
    ...
    css_class = Column(String(200), nullable=True, comment="CSS样式类")
    list_class = Column(String(100), nullable=True, comment="列表样式类")
    is_default = Column(
        Integer, nullable=False, default=0, comment="是否默认值：0-否，1-是"
    )
    status = Column(Integer, nullable=False, default=0, comment="状态：0-启用，1-禁用") 
    remark = Column(Text, nullable=True, comment="备注说明")
