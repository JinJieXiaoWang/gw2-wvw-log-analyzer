...
class SysUser(Base):
    ...
    username = Column(
        String(50), unique=True, index=True, nullable=False, comment="用户?
    )
    ...
    last_login = Column(DateTime(timezone=True), nullable=True, comment="最后登录时?)
    token_version = Column(
        Integer,
        default=0,
        nullable=False,
        comment="令牌版本（用于密码修改后使旧token失效?,
    )
